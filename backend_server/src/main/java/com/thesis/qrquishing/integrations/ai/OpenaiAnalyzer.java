 ...
 * <ul>
 *   <li>The OpenAI API key is read from the {@code OPENAI_API_KEY} environment
 *       variable — never hardcoded or logged.</li>
...
public class OpenaiAnalyzer implements AIAnalyzer{
    ...
    public ValidationResponse analyze(String url, UrlFeatures features) {
        ...
        try {
            ...
            if (response.statusCode() != 200) {
                throw new LlmException("OpenAI API returned HTTP " + response.statusCode());
            }

            return parseResponse(response.body(), features);
    ...
    /** Parse the OpenAI response and extract verdict + confidence. */
    private ValidationResponse parseResponse(String responseBody, UrlFeatures features) {
        try {
